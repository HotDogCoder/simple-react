...
function DarkModeSwitch(): JSX.Element {
    ...
    const [enabled, setEnabled] = useState<boolean>(darkTheme ?? false);
        ...
        return (
            <span onClick={handleMode}>
              ...
              relative inline-flex h-[36px] w-[74px] shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-500 ease-in-out focus:outline-none focus-visible:ring-2  focus-visible:ring-white focus-visible:ring-opacity-75`}
                ...
                pointer-events-none inline-block h-[32px] w-[34px] transform rounded-full bg-white dark:bg-slate-700 shadow-lg ring-0 transition duration-200 ease-in-out`}
                    >
                        {darkTheme ? (
                            <>
